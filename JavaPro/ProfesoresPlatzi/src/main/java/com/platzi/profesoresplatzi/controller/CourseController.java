...
public class CourseController {
	...
	public ResponseEntity<Course> getById(@PathVariable ("id") Long idCourse){
	...
	public ResponseEntity<?> createCourse(@RequestBody Course course, UriComponentsBuilder uriComponentsBuilder){
		if (course== null || course.getName()==null || course.getName().equals("") 
				|| course.getProject()==null || course.getProject().equals("")) {
			return new ResponseEntity(new CustomErrorType("Course name and project are required"), HttpStatus.CONFLICT);
		}
		else {
			...
			Course courseSaved = _courseService.findByName(course.getName());
			HttpHeaders headers = new HttpHeaders();
			headers.setLocation(
					uriComponentsBuilder.path("/v1/courses/{id}")
					.buildAndExpand(courseSaved.getIdCourse())
					.toUri());
	...
	//UPDATE
		...
		public ResponseEntity<?> updateCourse(@PathVariable("id") Long idCourse, @RequestBody Course course){
			...
			if(course.getName()==null || course.getThemes()==null)
				return new ResponseEntity(new CustomErrorType("Course name and themes are required"), HttpStatus.CONFLICT);
